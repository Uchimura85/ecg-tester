...
public class LineChartRenderer extends LineRadarRenderer {
    ...
    protected void drawDataSet(Canvas c, ILineDataSet dataSet) {
        ...
        if(dataSet.isDrawValuesEnabled()) drawValues(c);
    }
       ...
       // if (isDrawingValuesAllowed(mChart))
        {
            ...
            for (int i = 0; i < dataSets.size(); i++) {
                ...
                for (int j = 0; j < positions.length; j += 2) {
                    ...
                    if(entry.getData()==null || entry.getData() == "") continue;

                    mValuePaint.setColor(dataSet.getValueTextColor(j / 2));
                    c.drawCircle( x, y,4,mValuePaint);
                    String str = (String)entry.getData();
                    c.drawText(str, x, y - 3*valOffset, mValuePaint);
